...
HuggingFace Transformers (v4.36+) 原生支援 Flash Attention 2。
...
# 如果支援，則顯示 'flash_attention_2'
```

## 配置範例

### 帶 Flash Attention 的 Llama 2
...
### 帶 Flash Attention 的 Mistral 用於長上下文
...
model = AutoModelForCausalLM.from_pretrained(
    ...
    attn_implementation="flash_attention_2",
    torch_dtype=torch.bfloat16,  # 長上下文效果更好
    device_map="auto",
...
# 使用 Flash Attention 的模型並行
model = AutoModelForCausalLM.from_pretrained(
    "meta-llama/Llama-2-13b-hf",
    attn_implementation="flash_attention_2",
    torch_dtype=torch.float16,
    device_map="auto",  # 自動多 GPU 放置
...
| 模型 | Batch Size | 標準 | Flash Attn 2 | 加速比 |
|-------|------------|----------|--------------|---------|
...
檢查上方的支援清單。如果不支援，請使用 PyTorch SDPA 作為回退 (Fallback) 方案：

```python
model = AutoModelForCausalLM.from_pretrained(
    "model-name",
    attn_implementation="sdpa",  # PyTorch 原生 (仍然較快)
    torch_dtype=torch.float16
)
```

### 問題：載入期間發生 CUDA 顯存溢出 (OOM)
...
model = AutoModelForCausalLM.from_pretrained(
    ...
    torch_dtype=torch.float16,
    device_map="auto",
    max_memory={0: "18GB"},  # 為 KV 快取預留記憶體
...
Flash Attention 在數值上是等價的，但使用了不同的計算順序。微小差異 (<1e-3) 是正常的：
...
print(f"最大差異: {diff:.66f}")  # 應該在 ~1e-3 到 1e-4 左右
```

### 問題：模型載入期間發生 ImportError
...
1. **始終在 Flash Attention 中使用 float16/bfloat16** (不要使用 float32)
2. **設置 device_map="auto"** 以進行自動記憶體管理
3. **長上下文建議使用 bfloat16** (數值穩定性更好)
4. **訓練大型模型時啟用梯度檢查點 (Gradient checkpointing)**
...
model = AutoModelForCausalLM.from_pretrained(
    "meta-llama/Llama-2-7b-hf",
    attn_implementation="flash_attention_2",
    torch_dtype=torch.bfloat16,  # 訓練效果更好
    device_map="auto",
    low_cpu_mem_usage=True
)
